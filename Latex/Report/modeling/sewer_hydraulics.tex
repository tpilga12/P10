...
\underbrace{\left(\rho \cdot \frac{Q}{A}  \cdot Q + \frac{\partial}{\partial x}\left(\rho \cdot \frac{Q}{A}  \cdot Q\right) \cdot \frac{\Delta x}{2} \right)}_{F_{out}}
...
The hydrostatic forces acting on the channel are found by taking a sectional view. By assuming hydrostatic pressure across the channel, the pressure in a height z above the bottom of the channel is given as $g\rho(h-z)$, where h is the hight of the fluid.  
...
\end{equation}
Where $h_l$ is the fluid height to the left of the channel, b(z) is the width of the channel given z. The hydrostatic force acting on the control volume to the right is given as:
\begin{equation}\label{eq:P2}
\begin{array}{l}
	-\int_{0}^{h_r} \rho \cdot g (h_r - z)\cdot b(z)dz = \vspace{2mm}\\
	-\int_{0}^{h_l} \rho \cdot g (h_l - z)\cdot b(z)dz -\int_{0}^{h_l} \rho \cdot g (h_r- h_l)\cdot b(z)dz -\int_{h_l}^{h_r} \rho \cdot g (h_r - z)\cdot b(z)dz = \vspace{2mm}\\
	-F_{P1} - F_{P2} - F_{P3}
\end{array}	
\end{equation}
...
\end{equation}
Where $h_r$ is the fluid hight to the right of the channel. Integrating the width of the channel gives the area of the fluid in the channel. The rest in the integral is moved outside the integral as it is a constant. By assuming the width, b is constant for $F_{P3}$ the following can be approximated: 
\begin{equation}
