...
If an infinite small slice of cross section in the control volume is considered, and utilizing the product rule on the hydrodynamic force acting on the slice, the following is obtained:  
...
Where n is Manning's roughness factor $[s\cdot m^{-1/3}]$, f is the Weisbach resistance coefficient $[-]$ and R is the hydraulic radius $[m]$ given as the wetted area divided by the wetted perimeter \cite{stormwatercollectionsystems}.
...
By assuming hydrostatic pressure across the channel, the pressure in a height z above the bottom of the channel is given as $g\rho(h-z)$, where h is the height of the fluid.  
The forces acting on the left side of the control volume is given as:
\begin{equation}\label{eq:P1}
	F_{P1} = \int_{0}^{h_l} \rho \cdot g (h_l - z)\cdot b(z) dz
\end{equation}
Where $h_l$ is the fluid height in the left side of the control volume, b(z) is the width of the channel given the height z. The force acting on the right side of the control volume is given as:
...
\begin{equation}
\begin{aligned}
F_{P1} -F_{P1} -F_{P2} - F_{P3} =& -\rho\cdot g \cdot \frac{\partial h}{\partial x} \cdot \Delta x \left(A_l + \frac{1}{2}b(h)\frac{\partial h}{\partial x} \Delta x \right) \\
...
\fxnote{flet noget ind med at vi afgrænser os fra lateralt side input og laver samlings punkter mellem rørerene.} 
