...
\caption{Illustration of the control volume $\Omega$, where the marked end pieces illustrates inifinitely small slices, and v is the velocity of the wastewater.}
...
\begin{equation}\label{eq:force_equal_vrhoQ}
	F=\frac{d\mathcal{M}}{dt} =M \cdot \frac{dv}{dt} + \frac{dM}{dt} \cdot v 
\end{equation}

Where M is the mass of the control volume. The term $dM/dt$ is the mass in a infinite small slice going in and out of the control volume, and v is the velocity of this transition. Due to the assumption of incompressible fluid in the control volume the mass derivative term in equation \ref{eq:force_equal_vrhoQ} can be rewritten to:
