...
\caption{General overview of a wastewater treatment plant. arbejdsblad billede. \fxnote{Ny tegning + combustion er incineration plant}}
...
Skal have omformuleret dette afsnit, ikke særlig godt skrevet pt!

The WWTP in Fredericia purifies wastewater from the industry and the households. A general daily intake of wastewater is illustrated in figure   
...
It has a peak around where people start to wake up to prepare for work, where the majority of wastewater comes from toilets visits and baths. Doing the day the flow slows down until people start to return from work where it has a flow increase due to cooking, baths, baths etc. Doing night the flow is at a minimum due to people are sleeping, however there is a constant intake of precipitation through out the pipeline because it will sink through the pipe wall.  

These spikes or increased flows at certain times results in a lower quality of the effluent that leaves the WWTP. The aeration phase where the process requires power to drive the pumps that create this oxygenation of the water. This process will follow the input flow to accommodate the change. Because when the input is high the process requires a higher population of microorganisms to purify the wastewater, and therefore more air is needed in the wastewater. 

In Fredericia there are multiple big companies that empty their process out into the sewer. An example is the dairy. They are known to empty their process of ????? tons cream into the sewer. This a huge load for the WWTP to purify. Often the companies does not alert the WWTP about these load changes. This possess a huge problem WWTP ???? as it does not have the capacity to purify to achieve a acceptable output quality of the effluent. (Tømmer de bare ud? Eller bliver processen bare speeded up så det ikke bliver renset ligeså godt?). Furthermore the concentration of the matter that comes from the industry is also a problem for the WWTP, as it is in some cases very concentrated.  

It is desired to have as constant as possible input into the WWTP to keep the aeration phase as constant as possible. Thereby reducing the energy cost in this phase and gain a more optimal purify process. Thereby obtaining a better water quality at the output of the plant. Furthermore the it is wanted to attenuate the concentration of the matter from the industry to a level where the WWTP can handle it. 

It is suggested from the department of ???? (Amelia) that buffer tanks should be placed though out the network. The idea is to hold back the wastewater and thereby control the output of these tanks to obtain a desired flow. 
...
Måske noget om måle metoden. 

Vis noget med flowet over en dag

Løsningsmetode med at indsætte buffer tanke (Skal ikke være i introduktionen, medmindre der allerede er tanke så vil det være dem vi skal beskrive)

Beskriv styrelsen af dem
