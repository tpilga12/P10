...
At approximately 06:00 there is a steep increase in flow, which is due to people preparing for work.% where the majority of wastewater comes from toilets visits and baths.
...
These outlets posses a problem for the WWTP. Not only because of the change in flow, but also because the concentrate of the various compounds in the wastewater is usually higher from the larger companies. Typically the industry is efficient in letting out wastewater within certain pH levels at reasonable flow levels. The only counter measure, for these incidents of a sudden discharge, to prevent overflow is to skip processes and let the wastewater through to the sedimentation phase \fxnote{is this true? der står mekaniske rensning ned i referatet} and then into the fjord.
