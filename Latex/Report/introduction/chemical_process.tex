...
It has a peak around where people start to wake up to prepare for work, where the majority of wastewater comes from toilets visits and baths. Doing the day the flow slows down until people start to return from work where it has a flow increase due to cooking, baths, baths etc. Doing night the flow is minimum due to people are sleeping, however there is a constant intake of precipitation through the pipeline because it will enter into through the pipe wall.  
...
In Fredericia there are multiple big companies that empty their process out into the sewer. An example is the dairy they are known to empty their process of ????? tons cream into the sewer. This a huge load for the WWTP to purify.  

It is wanted to have a smooth as possible input into the WWTP to keep this aeration phase as constant as possible. Thereby reducing the energy cost in this phase and gain a more optimal purify process. Thereby obtaining a better water quality at the output of the plant.

It is suggested from the department of ???? (Amelia) that buffer tanks should be placed though out the network. These tanks should have a output flow which is controlled. Hence 
