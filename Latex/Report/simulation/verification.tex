...
The playback function is constructed such that firstly minimum and maximum values on the y axis is limited to the variations in the data. This apply for all pipes and tanks in the system. Secondly, ``Start\_point'' is set to the iteration from where the playback is to start from, ``step\_size'' is set to the amount of iterations to skip during playback. Lastly ``frame\_time'' is set, which decides the speed of the playback. The maximum speed is in the end decided by the computational speed available to MATLAB and due to the plot function being computational demanding a low amount of updates per second should be expected.
