...
Two functions, namely ``Tank'' and ``Init\_pipe'', is used to obtain the initial value for tanks and the boundary conditions needed to start simulating with the Preissmann scheme. The tank function returns the initial flow and height together with the input needed for the pump, such that inflow is equal to outflow in the tank. The pipe function takes initial flow and something else, and yes this sentence is going to change.
