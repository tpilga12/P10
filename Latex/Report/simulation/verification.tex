...
As mentioned in section \ref{sec:tank} considerations is needed when fluid height goes towards zero. As seen in figure \ref{fig:tank_function} if the calculated height gives a result below zero the height is set to zero and maximum outflow is set equal to inflow. In the case of the concentrate, a height limit is set when solving equation \ref{eq:disc_con_tank}  
