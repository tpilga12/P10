...
Where k is the number of iterations, V$'$ is the differentiated of V with respect to height, $(h_{j+1}^{i+1})_{k}$ is an initial guess of the root and $(h_{j+1}^{i+1})_{k+1}$ is an approximate of the height. This calculation is iterated until a satisfactory approximation is achieved which fulfills the requirement,
