...
However, to make the constraint depend on $\Delta \CMcal{U}$ the difference between the current timestep and the previous timestep, $\Delta \CMcal{U}(k) =  \CMcal{U}(k)- \CMcal{U}(k-1)$, is inserted on the state, input and disturbance: 

\begin{equation}
     \underline{x}- \bar{x} \leq \CMcal{A}(\Delta\hat{x}(k)+ x(k-1))+\CMcal{B}(\Delta\CMcal{U}(k)+\CMcal{U}(k-1))+\CMcal{B}(\Delta\CMcal{D}(k)+\CMcal{D}(k-1)) \leq \overline{x}-\bar{x}
 \end{equation} 
