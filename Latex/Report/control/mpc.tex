...
\subsection*{Results}
In the subsection the results obtained from the testing of the MPC controller will be covered. Two test will conducted, one where the constraints are neglected and therefore minimized without any restriction and another test where the constraints included. 
...
In the first test the MPC controller is minimizing the output variations of the tank without any constraints included. The input signal in the first pipe is shown in figure \ref{fig:input_to_pipe_mpc_test}. 
\begin{figure}[H]
 \centering
...
From figure ??? it can be seen that the output of the tank is constant which fulfills the cost function problem described 
