...
However, to make the constraint depend on $\Delta \CMcal{U}$ the difference between the current time step and the previous time step, $\CMcal{U}(k+n) = \CMcal{U}(k-1) \sum_{i=1}^n\Delta \CMcal{U}(i) $, is inserted on the : 
...
\begin{equation}
 ...
 \begin{equation}
       \underbrace{\underline{x}- \bar{x}-\CMcal{A}(\Delta\hat{x}(k)+ x(k-1))-\CMcal{B}(\Delta\CMcal{D}(k)+\CMcal{D}(k-1))-\CMcal{B}\CMcal{U}(k-1)}_{x_1}\  \leq \CMcal{B}\Delta\CMcal{U}
 \end{equation}  
