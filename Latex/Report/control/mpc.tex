...
Where $\CMcal{X}$ is the predicted state vector for the entire prediction horizon. $\CMcal{A}$ is the state matrix up to the prediction horizon. $x(k|k)$ is the initial state and is used to predict the hole prediction horizon, $\CMcal{B}$ is the input matrix for the prediction horizon, $\CMcal{U}$ is the predicted input vector, which consists of all the predicted inputs from the current time step until $(k+H_p-1)$. $\CMcal{B}_d$ is the disturbance matrix for the prediction horizon and $\CMcal{D}$ is the disturbance vector. 
...
Furthermore, it is desired to have the control input as $\Delta \CMcal{U}$ as this will introduce integral action and thereby eleminate steady state error\fxnote{kilde enten til john eller bog}. To do so, equation \ref{eq:lifted_output_with_states_inserted} is inserted into equation \ref{eq:differnce_between_y} thereby obtaining:
...
However, to make the constraint depend on $\Delta \CMcal{U}$ the difference between the current time step and the previous time step, $\CMcal{U}(k) = \CMcal{V}u(k-1) +W\Delta \CMcal{U}(k) $, where $\CMcal{V}$ is a vector and W is a matrix on the form: 
...
%  \end{equation} 
 ...
 Now the constraint is set up as a constraint on the input signal. In addition, the constraint has to be set up as equality constraints, therefore, it is divided into two constraints on for the upper bound and one for the lower bound: 

 \begin{equation}
 \begin{aligned}
 x_{max}-\bar{x} &\geq \CMcal{A}\hat{x}(k)+\CMcal{B}\CMcal{V}\hat{u}(k-1) +W\Delta \hat{\CMcal{U}}(k)+\CMcal{B\hat{D}}(k)\\
 \underbrace{BW}_{\Lambda} \Delta\hat{\CMcal{U}}(k)&\leq \underbrace{x_{max}- \bar{x}-\CMcal{A}\hat{x}(k)-\CMcal{B}\CMcal{V}\hat{u}(k-1)-\CMcal{B}\hat{\CMcal{D}}(k)}_{\Gamma_1}
...
\end{aligned}
 ...
 \begin{equation}
 \begin{aligned}
 x_{min}- \bar{x} &\leq \CMcal{A}\hat{x}(k)+\CMcal{B}\CMcal{V}\hat{u}(k-1) +W\Delta \hat{\CMcal{U}}(k)+\CMcal{B\hat{D}}(k)\\
