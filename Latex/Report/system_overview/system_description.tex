...
From figure \ref{fig:sewer_line_diagram} it can be seen that 11 different flow profiles are to be constructed, to cover wastewater contribution to the main line from each residential and industrial area. %Each of these residential and industrial areas have been measured to give an estimate of the area for each zone. 
...
\begin{tabular}{|c|c|c|c|}
\hline
\multicolumn{1}{|c|}{\cellcolor[HTML]{9B9B9B}\textbf{Zone}} & 
\multicolumn{1}{|c|}{\cellcolor[HTML]{9B9B9B}\textbf{\begin{tabular}[c]{@{}c@{}}Residential\\ area $[km^2]$\end{tabular}}} & 
\multicolumn{1}{|c|}{\cellcolor[HTML]{9B9B9B}\textbf{\begin{tabular}[c]{@{}c@{}}Industrial\\ area $[km^2]$\end{tabular}}} & 
\multicolumn{1}{|c|}{\cellcolor[HTML]{9B9B9B}\textbf{\begin{tabular}[c]{@{}c@{}}Population \\per area \end{tabular}}}\\ \hline
...
In figure \ref{fig:flow_profile_thulevej} a flow profile is shown for residential area 1,1. 
...
\begin{figure}[H]
	...
	\input{report/system_overview/tikz/brewery_flow}
	\caption{Combined outflow from brewery and bottling plant during 24 hours.}
	\label{fig:flow_profile_industry}
\end{figure}	

From the figure, it can be seen that the combined outflow from the brewery and bottling plant arrives in pulses of up to 200 $m^3/hr$. The longest pulse last approximately an hour and the shortest approximately 10 minutes. The duration between ranges approximately from three minutes to one and a half hour. 
%From the flow pattern in the figure, it is likely indicated that there are two pumps pumping the wastewater away from the industry, as there are two levels in the plot. Around 190-200 $m^3/hr$ and around 110-120 $m^3/hr$. 
...
\begin{figure}[H]
\centering
\input{report/system_overview/tikz/flow_input_wwtp}
\caption{Typical dry weather Inflow at Fredericia wastewater treatment plant during 24 hours.}
\label{fig:flow_input_wwtp}
\end{figure}  

%In the figure it can be seen how the inflow to the WWTP fluctuates. From 00:00 to 03:25 the flow fluctuates between 1000 $m^3/hr$ to 1200 $m^3/hr$ and these huge shirts in flow rate is ongoing the entire day. Around 07:00 a large shift in flow occurs, it is unknown if this is a false measurement or it is due to a huge load change, as this peck seems to be only for one sample. However, it is very clear that the flow fluctuates across the entire day and it would be desirable to smooth the flow out.    
Unfortunately, concentrate measurements were not available from the industry. For that reason, it is decided to lessen the focus on the various chemicals and implement it as a single component. Measurements are available from the inflow to the WWTP, but due to dispersion of the concentrate during transport from the industry to the WWTP, it is not possible to recreate a proper profile for the industry. 
In appendix \ref{app:flow_profiles} figure \ref{fig:COD_data}, \ref{fig:Forfos_data} and \ref{fig:kvaelstof_data} show COD, phosphorus and nitrogen measurements respectively at the inflow of the WWTP. 

%These figures show a similar pattern as the flow measurement, however, they do not vary as much as the flow. But a direct comparison cannot be done, as these measurements, the concentration measurements and the flow measurement, are measured at different dates, and therefore it can not be concluded if these concentrations are varying as much as the flow measurement. However, it can be concluded that these measurements also fluctuates across a day and therefore needs to be smoothed out to obtain a better cleansing process at the WWTP. 
