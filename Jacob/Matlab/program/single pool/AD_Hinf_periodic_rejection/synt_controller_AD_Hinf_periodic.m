...
% In this file, we compute an Hinfinity controller rejecting a periodic perturbation  
%
% A simulink simulator is aslo associated to this file:   simulation_rejection_periodic.mdl 
%
...
% Weighting function allowing to remove a perturbation acting at wf;
%
%            s^2 + 2\xin wf + wf^2
%  W1f (s)= -----------------------
%            s^2 + 2\xid wf + wf^2
...
xin=2;
xid=0.001;
numWp1=[1 2*xin*wf wf^2];
denWp1=[1 2*xid*wf wf^2];
Wp1=ss(tf(numWp1,denWp1));
...
[K_hinf_ss,CL,gamma] = hinfsyn(sys_aug,1,1,'GMAX',100, 'GMIN',0,'TOLGAM',0.01,'METHOD','ric','DISPLAY','on');
...
%
figure(4)
omega=sort([logspace(-1.8,-2.2,500),logspace(-3,3,100)]);
