function [output]=init(piping,input)
...
%         Qf = -3.02 * log((0.74*10^(-6))/(d*sqrt(d*Ie(m,n)))+(k/(3.71*d)))*d^2*sqrt(d*Ie(m,n)); %[m^3/s] palles
        ...
        if x == 1
            ...
            data{x}.h(1:sections) = fitfunc.p1*Q_init.^9 +fitfunc.p2*Q_init.^8 + fitfunc.p3*Q_init.^7 + fitfunc.p4*Q_init.^6 + fitfunc.p5*Q_init.^5 + fitfunc.p6*Q_init.^4 + fitfunc.p7*Q_init.^3 + fitfunc.p8*Q_init^2 + fitfunc.p9*Q_init +fitfunc.p10;
...
%                         h(1:sections) = init_height(epsi,Q(1,1),Q_mark,0,d);
            data{x}.h(1:sections) = fitfunc.p1*Q_in.^9 +fitfunc.p2*Q_in.^8 + fitfunc.p3*Q_in.^7 + fitfunc.p4*Q_in.^6 + fitfunc.p5*Q_in.^5 + fitfunc.p6*Q_in.^4 + fitfunc.p7*Q_in.^3 + fitfunc.p8*Q_in^2 + fitfunc.p9*Q_in +fitfunc.p10;
            else
                data{x}.h(1:sections) = fitfunc.p1*data{x-1}.Q(1,end).^9 +fitfunc.p2*data{x-1}.Q(1,end).^8 + fitfunc.p3*data{x-1}.Q(1,end).^7 + fitfunc.p4*data{x-1}.Q(1,end).^6 + fitfunc.p5*data{x-1}.Q(1,end).^5 + fitfunc.p6*data{x-1}.Q(1,end).^4 + fitfunc.p7*data{x-1}.Q(1,end).^3 + fitfunc.p8*data{x-1}.Q(1,end)^2 + fitfunc.p9*data{x-1}.Q(1,end) + fitfunc.p10;
            end  
        end
        data{x}.A(1:sections) = d^2/4 * acos(((d/2)-h(n))/(d/2))-sqrt(h(n)*(d-h(n)))*((d/2)-h(n));
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Border conditions %%%%%%%%%%%%%%%%%%%%
    elseif m > 1 && n == 1
        if x == 1
            ...
            data{x}.h(m,n) = fitfunc.p1*Q_in.^9 +fitfunc.p2*Q_in.^8 + fitfunc.p3*Q_in.^7 + fitfunc.p4*Q_in.^6 + fitfunc.p5*Q_in.^5 + fitfunc.p6*Q_in.^4 + fitfunc.p7*Q_in.^3 + fitfunc.p8*Q_in^2 + fitfunc.p9*Q_in +fitfunc.p10;
        else
...
%             data{x}.h(m,n) = init_height(epsi,Q(1,1),Q_mark,0,d);
                        data{x}.h(m,n) = fitfunc.p1*Q(m,n).^9 +fitfunc.p2*Q(m,n).^8 + fitfunc.p3*Q(m,n).^7 + fitfunc.p4*Q(m,n).^6 + fitfunc.p5*Q(m,n).^5 + fitfunc.p6*Q(m,n).^4 + fitfunc.p7*Q(m,n).^3 + fitfunc.p8*Q(m,n)^2 + fitfunc.p9*Q(m,n) +fitfunc.p10;
...
%                 h(m,n)=NewtonRoot(@V,@V_dot,data{x-1}.h(m-1,end),limitvalue,50,d,Ie,H,Dt,Dx,Theta,m,n);
        ...
        data{x}.A(m,n) = d^2/4 * acos(((d/2)-h(m,n))/(d/2))-sqrt(h(m,n)*(d-h(m,n)))*((d/2)-h(m,n));
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Iteration %%%%%%%%%%%%%%%%%%%%%%%%%%%
    elseif m > 1 && n > 1 
        ...
        data{x}.h(m,n)=NewtonRoot(@V,@V_dot,h(m-1,n-1),limitvalue,50,d,Ib,H,Dt,Dx,Theta,m,n);
        data{x}.A(m,n) = d^2/4 * acos(((d/2)- h(m,n))/(d/2))-sqrt(h(m,n)*(d-h(m,n)))*((d/2)-h(m,n));
        data{x}.Q(m,n) = (-1/(Theta*2))*(A(m,n)-H)*Dx/Dt;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % CONCENTRATE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%% euler baglaens %%%%
        %         C(m,n)= (C(m,n-1)*A(m,n))/(A(m,n)+Q(m,n)*(Dt/Dx))+(Q(m,n)*C(m-1,n))/(A(m,n)*(Dx/Dt)+Q(m,n));
        data{x}.C(m,n)= (C(m-1,n)*A(m,n))/(A(m,n)+Q(m,n)*(Dt/Dx))+(Q(m,n)*C(m,n-1))/(A(m,n)*(Dx/Dt)+Q(m,n));
        %%%%%%%%%%%%%%%%%%%%%%%
        
    end
end

for j = 1:length(pipe_spec)
