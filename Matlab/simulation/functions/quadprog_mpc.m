function [X,FVAL,EXITFLAG]=quadprog_mpc(gamma,psi,Q,delta_xstates,A_constraints, b_constraints,Alifted,Bulifted,xstates_old,u_output_tank_old,Test_matrix,xstates,C_matrix_mpc,input)
...
%  b_constraints1(:,1) = b_constraints(:,1)-(Alifted*(xstates')-Bulifted*u_output_tank_old);
 b_constraints1(:,1) = b_constraints(:,1)-(Alifted*(xstates')-Bulifted*ones(120,1)*u_output_tank_old);
...
%  [X,FVAL,EXITFLAG] = quadprog(H,f,[-Test_matrix; Test_matrix],[u_output_tank_old+input.u_init(1,1) 1-input.u_init(1,1)-u_output_tank_old],[],[],[],[],[],options);
%  [X,FVAL,EXITFLAG] = quadprog(H,f,[Bulifted*Test_matrix],[ b_constraints1(:,1)],[],[],[],[],[],options);
 
% [X,FVAL,EXITFLAG] = quadprog(H,f,[-Test_matrix; Test_matrix; C_matrix_mpc*Bulifted*Test_matrix],[u_output_tank_old+input.u_init(1,1) ; 1-input.u_init(1,1)-u_output_tank_old; C_matrix_mpc*b_constraints1(:,1)  ],[],[],[],[],[],options);
[X,FVAL,EXITFLAG] = quadprog(H,f,[-Test_matrix; Test_matrix; C_matrix_mpc*Bulifted*Test_matrix],[u_output_tank_old*ones(120,1)+input.u_init(1,1) ; 1-input.u_init(1,1)-u_output_tank_old*ones(120,1); C_matrix_mpc*b_constraints1(:,1)  ],[],[],[],[],[],options);
