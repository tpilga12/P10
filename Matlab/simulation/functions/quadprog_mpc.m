...
% [X,FVAL,EXITFLAG] = quadprog(H,f,[-SUM_matrix_mpc; SUM_matrix_mpc; C_matrix_mpc*Bulifted*SUM_matrix_mpc],[u_output_tank_old*ones(size(Bulifted,2),1)+input.u_init(1,1)-0.2 ;0.9-input.u_init(1,1)-u_output_tank_old*ones(size(Bulifted,2),1); C_matrix_mpc*b_constraints1(:,1)  ],[],[],[],[],[],options);
 [X,FVAL,EXITFLAG] = quadprog(H,f,[-SUM_matrix_mpc; SUM_matrix_mpc; C_matrix_mpc*Bulifted*SUM_matrix_mpc;-C_matrix_mpc*Bulifted*SUM_matrix_mpc],[u_output_tank_old*ones(size(Bulifted,2),1)+input.u_init(1,1);0.9-input.u_init(1,1)-u_output_tank_old*ones(size(Bulifted,2),1); C_matrix_mpc*b_constraints1(:,1);C_matrix_mpc*b_constraints1(:,2)  ],[],[],[],[],[],options);
