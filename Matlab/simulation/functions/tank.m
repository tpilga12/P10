function [data] = tank(m,tank_nr,Q_in,C_in,input,tank_spec,init)
...
if init == 1
    ...
    data.h(m,1) = (1/tank_spec(tank_nr).area)*(data{1}.Q(m,1)-data{1}.Q(m,2))*Dt;
    data.C(m,1) = C_in;
    if data.h(m,1) <= 0
        data.h(m,1) = 0;
        data.Q(m,2) = 0;
    elseif data.h(m,1) > tank_spec(m).height
        data.h(m,1) = tank_spec(m).height;
