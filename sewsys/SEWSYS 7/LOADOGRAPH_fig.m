function varargout = LOADOGRAPH_fig(varargin)
% LOADOGRAPH_FIG M-file for LOADOGRAPH_fig.fig
%      LOADOGRAPH_FIG, by itself, creates a new LOADOGRAPH_FIG or raises the existing
%      singleton*.
%
%      H = LOADOGRAPH_FIG returns the handle to a new LOADOGRAPH_FIG or the handle to
%      the existing singleton*.
%
%      LOADOGRAPH_FIG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LOADOGRAPH_FIG.M with the given input arguments.
%
%      LOADOGRAPH_FIG('Property','Value',...) creates a new LOADOGRAPH_FIG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before LOADOGRAPH_fig_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to LOADOGRAPH_fig_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help LOADOGRAPH_fig

% Last Modified by GUIDE v2.5 27-Sep-2004 13:36:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @LOADOGRAPH_fig_OpeningFcn, ...
                   'gui_OutputFcn',  @LOADOGRAPH_fig_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before LOADOGRAPH_fig is made visible.
function LOADOGRAPH_fig_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to LOADOGRAPH_fig (see VARARGIN)

% Choose default command line output for LOADOGRAPH_fig
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes LOADOGRAPH_fig wait for user response (see UIRESUME)
% uiwait(handles.figure1);
global v
v=2;

% --- Outputs from this function are returned to the command line.
function varargout = LOADOGRAPH_fig_OutputFcn(hObject, eventdata, handles)
...
HH8=findobj(gcf,'Tag','acc_load');
acc_choice=get(HH8, 'Value');
if acc_choice==1
    s=size(T);
    acc1=storm(1,v)*sec_per_ts/1000;
    for i=1:1:s(1)-1
        %i
        j=i+1;
        %size(acc1)
        acc2=storm(j,v)*sec_per_ts/1000;
        acc1=[acc1;acc1(i)+acc2];
    end
    plot(T,acc1,'m')
    xlim([in_first in_last])
    HH5 = findobj(gcf,'Tag','subname');
    text1=get(HH5, 'String');
    fil=sprintf('%s %s %s %s %s %s %s','Accumulated Loadograph:',text1);
    title(fil)                                                                                                          
    xlabel('Timesteps')                                                                                                         
    ylabel('kg')
else
    ...
    HH5 = findobj(gcf,'Tag','subname');
    text1=get(HH5, 'String');
    fil=sprintf('%s %s %s %s %s %s %s','Loadograph:',text1);
    title(fil)                                                                                                          
    xlabel('Timesteps')                                                                                                         
    ylabel('g/s')
end
...
% --- Executes on button press in acc_load.
function acc_load_Callback(hObject, eventdata, handles)
% hObject    handle to acc_load (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of acc_load
